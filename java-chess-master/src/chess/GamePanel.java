...
public class GamePanel extends JComponent implements MouseListener {
    private Stack<Board> futureStates = new Stack<Board>();
    ...
    final Color validMoveColor = new Color(0,0,0,100);
    ...
    final Color darkColor = new Color(0,255,0,127);
    ...
    public void undo() {
        if (gameBoard.getPreviousState() != null) {
            // Push the current state to the future states stack before undoing
            futureStates.push(gameBoard);

            // Reset variables for helper circles
            selectedPiece = null;
            invalidPiece = null;
            okMoves = null;
    ...
    public void redo() {
        if (!futureStates.isEmpty()) {
            // Pop the future state and make it the current state
            gameBoard = futureStates.pop();
            // Repaint the board
            this.repaint();
        }
    }
    ...
    private void drawHelperCircles(Graphics g, int sW, int sH) {
        ...
        if(selectedPiece != null) {
            ...
            // and all of its valid moves
            g.setColor(validMoveColor.brighter()); // make the color lighter
            for(Move m : okMoves) {
                Point pt = m.getMoveTo();
                g.fillOval(pt.x * sW + sW/4, pt.y * sH + sH/4, sW/2, sH/2); // draw smaller circle
            }
        }
